...
if [ -d ${EXT_DIR} ]; then                                      ## Test to see if external drive directory exists
    if [ ! -d "$EXT_DIR/wkdu/$YR-$MO-$DAY" ]; then              ## Check for existing directory for today, if not make one
        echo $NOW "Creating directory in external drive: $EXT_DIR/wkdu/$YR-$MO-$DAY"
        mkdir ${EXT_DIR}/wkdu/$YR-$MO-$DAY
    fi
else
    EXT2=`df -h | grep '/dev/sdc2' | awk '{print $6}'`          ## Check for a different external drive location since it was unable to find the default
    if [ -d ${EXT2} ]; then
        EXT_DIR=EXT2
    ...
    else if [ ! -d "/data/recordings/$YR-$MO-$DAY/"]; then      ## Still couldn't find external drive, let's check for a folder on the computer's internal drives
...
if [ -d ${EXT_DIR} ]; then
    ...
    arecord -D "plughw:1,0" -d3600 -f cd -t raw | lame -r -b 192 - ${EXT_DIR}/wkdu/$YR-$MO-$DAY/$YR-$MO-$DAY-$H$M.mp3             # Records audio stream for 60 minutes (3600 seconds)
else
    ...
    arecord -D "plughw:1,0" -d3600 -f cd -t raw | lame -r -b 192 - /data/recordings/$YR-$MO-$DAY/$YR-$MO-$DAY-$H$M.mp3              # Records audio stream for 60 minutes (3600 seconds)
fi
