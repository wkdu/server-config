...
if [ -d "$EXT_DIR" ]; then
    ...
    if [ ! -d "$EXT_DIR/wkdu/$YR-$MO-$DAY" ]; then
         ...
         echo "$NOW [192] Current day's recording directory does not exist. Creating directory: $YR-$MO-$DAY"
    fi

    echo "$NOW [192] Started recording audio file to external drive: $EXT_DIR/wkdu/$YR-$MO-$DAY/$YR-$MO-$DAY-$H$M.mp3"

    ## Records audio stream for 60 minutes (3600 seconds)
    arecord -D plug:snoop -d3600 -f cd -t raw -q | lame -m s -r -b 192 - "$EXT_DIR/wkdu/$YR-$MO-$DAY/$YR-$MO-$DAY-$H$M.mp3" --quiet
    ...
    echo "$NOW [192] Unable to find external drive during create-recording cronjob. Started recording in internal drive instead: /data/recordings/$YR-$MO-$DAY/$YR-$MO-$DAY-$H$M.mp3" | mail -a "From: Recordings Server <recordings@wkdu.org>" -s "[ALERT] Created new recording in INTERNAL drive" admin@wkdu.org
    echo "$NOW [192] Started recording audio file to internal drive: /data/recordings/$YR-$MO-$DAY/$YR-$MO-$DAY-$H$M.mp3 (could not find external drive)"

    ## Records audio stream for 60 minutes (3600 seconds)
    arecord -D plug:snoop -d3600 -f cd -t raw -q | lame -m s -r -b 192 - "/data/recordings/$YR-$MO-$DAY/$YR-$MO-$DAY-$H$M.mp3" --quiet

fi

NOW=$(date +'%Y/%m/%d %T')                                              ## resets date / time variable $NOW to current time
echo "$NOW [192] Finished recording audio file: $YR-$MO-$DAY-$H$M.mp3"        ## puts stop recording time entry in record.log
